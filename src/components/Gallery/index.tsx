  ...
  return (
    <div className='sm:h-[85vh] h-auto sm:w-auto w-11/12 mx-auto scroll-smooth overflow-x-scroll overflow-y-hidden scrollbar-hide'>
      <div className='sm:h-full h-auto sm:w-auto w-full flex sm:flex-row flex-col'>
        { renderedPhotos }
        <div className='relative sm-min-h-auto min-h-100px sm:h-full h-auto sm:min-w-200px sm:w-auto w-full sm:pr-10'>
          <button className='h-full w-full font-montserrat font-light text-sm text-center bg-neutral-200 hover:bg-neutral-300 transition-colors text-black' onClick={ () => setRenderBuffer(renderBuffer + 5) }>Load more</button>
        </div>
      </div>
    </div>
  )
